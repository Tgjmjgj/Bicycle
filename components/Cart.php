...
class Cart
{
    ...
    public static function addProduct($id)
    {
        ...
        if($product['availability']>0){
			...
			//echo "<script> alert('".json_encode($product)."')</script>";
            	if($productsInCart[$id]<$product['availability'])
				    $productsInCart[$id]++;
			    else
				    echo "<script> alert('Превышено максимальное количество товаров на складе')</script>";
            } else {
